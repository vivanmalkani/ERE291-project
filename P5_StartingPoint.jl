...
for jj=1:d
StartGridEnergy=GridEnergyRange[jj]
...
@variable(m,GridEnergy[t=1:T]>=0,start=StartGridEnergy)

#WindEnergy we use to pump water! [MWh]
@variable(m, SaveWindEnergy[t=1:T] >= 0,start=StartSaveWindEnergy)
...
display(ii)
display(jj)
BASEMAXREVENUE[ii,jj]=getobjectivevalue(m)
...
####################################################################
CSV.write("BaseCaseStartingPoints.csv",  DataFrame(BASEMAXREVENUE), writeheader=false)
