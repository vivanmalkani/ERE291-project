...
EFFICIENCYSENSITIVITY=zeros(d,d)

#Change in number initialize
TurbineEfficiencyRange=range(0.5,stop=0.95,length=d)
#Change in radius
PumpEfficiencyRange=range(0.5,stop=0.95,length=d)

for ii=1:d
TurbineEfficiency=TurbineEfficiencyRange[ii]

for jj=1:d
PumpEfficiency=PumpEfficiencyRange[jj]


OVERALLTURBINEEFFICIENCY=TurbineEfficiency #This includes the Penstock, Turbine, Nozzle, Generator, and Motor Efficiency.
OVERALLPUMPEFFICIENCY=PumpEfficiency #This reflects Penstock, pump, motor, and transformer efficiencies.
...
display(jj)
println("---------------------------")
EFFICIENCYSENSITIVITY[ii,jj]=getobjectivevalue(m)
#end loop for saved wind energy
end
#end loop for grid energy absorbed
end
...
####################################################################
#CSV.write("EfficiencySensitivityHatchett.csv",  DataFrame(EFFICIENCYSENSITIVITY), writeheader=false)
