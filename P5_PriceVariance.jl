...
WINDENERGY=data[1:T,4]
WINDPRICEORIGINAL=data[1:T,3]
MEANWINDPRICE = mean(WINDPRICEORIGINAL)
WINDPRICEDEVIATION = WINDPRICEORIGINAL - MEANWINDPRICE*ones(T,1)
...
VARYINGPRICEMAXREVENUE = zeros(d,1)
VARYINGREVENUEFROMWIND = zeros(d,1)

#Change in max passive storage height
PriceVarianceRange=-1:0.1:1

for ii = 21:21
VarianceLevel = 2^(PriceVarianceRange[ii])
WINDPRICE = MEANWINDPRICE*ones(T,1) + VarianceLevel*WINDPRICEDEVIATION
VARYINGREVENUEFROMWIND[ii] = sum(WINDPRICE[t]*WINDENERGY[t] for t=1:T)
...
solve(m)
display(ii)
VARYINGPRICEMAXREVENUE[ii]=getobjectivevalue(m)
...
#temp = [VARYINGPRICEMAXREVENUE,VARYINGREVENUEFROMWIND]
#writedlm("PriceVarianceAlt.csv",temp, ',')

PriceVarianceRevenuePlot = plot(PriceVarianceRange,
VARYINGPRICEMAXREVENUE,
ylabel = "Revenue (USD)",
 xlabel = "log2(Scaled Variance Level)",
 label = ["Revenue Varying with Price Profile Variance"],
 title="Price Variance Sensitivity",
 lw = 2)

png(PriceVarianceRevenuePlot, "Price Variance Sensitivity")
